...
@section('container')
    <div class="bg-slate-800 w-full h-max rounded-lg flex flex-col text-slate-300">
        ...
        <div class="p-4 mb-4">
            @if(session('success'))
            <div class="bg-slate-700 border border-lime-300 text-lime-300 rounded-lg w-full my-2 p-3">
                <p>{{ session('success') }}</p>
            </div>
            @endif
            <form method="post" action="/staff/admin/library/{{ $library->id }}" class="flex flex-col gap-3 w-full">
                @csrf
                @method('put')
                ...
                <div class="flex justify-end">
                    <button type="submit" class="bg-sky-500 rounded-lg p-2 text-white font-medium hover:bg-sky-400 hover:duration-150">Update Library Info</button>
                </div>
            </form>
        </div>
    </div>
